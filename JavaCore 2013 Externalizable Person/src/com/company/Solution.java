...
2013 Externalizable Person
The Person class must be serialized using the Externalizable interface.
Fix serialization error.
Method signatures cannot be changed.

Requirements:
1. The Solution.Person class must support the Externalizable interface.
2. The readExternal and writeExternal methods must allow you to correctly serialize and deserialize Person-type objects.
3. In the Solution.Person class, a constructor without parameters must be created.
4. In the Solution.Person class, a constructor with three parameters (String firstName, String lastName, int age) must be created.
